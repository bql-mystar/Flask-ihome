...
class User(BaseModel, db.Model):
    ...
    def password(self, origin_password):
        '''
        设置属性
        :param origin_password: 原始的，未加密的前的密码
        :return: 无返回值
        '''
    ...
    def to_dict(self):
        ...
        user_dict = {
            ...
            'name': self.name,
            'mobile': self.mobile,
            'avatar': constants.FDFS_BASE_URL + self.avatar_url if self.avatar_url else "",
...
class House(BaseModel, db.Model):
    ...
    def to_basic_dict(self):
        ...
        basic_dict = {
            ...
            'img_url': constants.FDFS_BASE_URL + self.index_image_url if self.index_image_url else '',
            'room_count': self.room_count,
            'order_count': self.order_count,
            'address': self.address,
            'user_avatar': constants.FDFS_BASE_URL + self.user.avatar_url if self.user.avatar_url else '',
    ...
    def to_full_dict(self):
        ...
        for image in self.images:
            img_urls.append(constants.FDFS_BASE_URL + image.url)
...
class Order(BaseModel, db.Model):
    ...
    status = db.Column(  # 订单的状态
        ...
        default="WAIT_ACCEPT", index=True)      # index指明在MySQL中这个字段建立索引，加快查询速度
    comment = db.Column(db.Text)  # 订单的评论信息或者拒单原因
    trade_no = db.Column(db.String(80))     # 支付宝的交易流水号
    ...
    def to_dict(self):
        ...
        order_dict = {
            ...
            "img_url": constants.FDFS_BASE_URL + self.house.index_image_url if self.house.index_image_url else "",
